...
public:
    ...
    std::vector<SearchResult> search(const std::string& query,
        ...
        // WASM: no background threads — skip async prefetch.
        // Stream URLs are fetched on demand in get_stream_formats().
        (void)prefetch_top;
    ...
    VideoInfo get_stream_formats(const std::string& video_id) {
        ensure_visitor_data();
        if (auto c = url_cache_.get(video_id)) return *c;   // copy, no dangling ptr
    ...
    void prefetch(const std::string& video_id) {
        // WASM: synchronous inline warm (no background threads)
        try {
            if (!url_cache_.has(video_id))
